...
function ascciArtFromFile(bf, PALATTE, LETTERS_PER_PIXEL, log) {
...
function getBitmapSorted(bitmap, data) {
    ...
    for (var i = 0; i < bitmap.length / 4; i++) {
        ...
        if (i % data.width == 0)
            bitmapRGBA.push([]);
        var pixel = bitmap.subarray(i * 4, (i + 1) * 4);
...
function brightToLetter(brightness, range, base, LETTER_PALETTE) {
    ...
    if (pos == LETTER_PALETTE.length)
        pos = LETTER_PALETTE.length - 1;
